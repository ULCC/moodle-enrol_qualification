...
class enrol_qualification_handler {
    public function role_assigned($ra) {
        ...
        if (!$enrols = $DB->get_records('enrol',
                                        array('customint1' => $parentcontext->instanceid,
                                              'enrol' => 'qualification'),
                                        'id ASC')) {
            return true;
        }
        ...
        foreach ($enrols as $enrol) {
            ...
            if (!$DB->record_exists('user_enrolments', array('userid' => $ra->userid,
                                                            'enrolid' => $enrol->id))) {
                continue;
            }
    ...
    public function role_unassigned($ra) {
        ...
        // note: do not check 'nosyncroleids', somebody might have just enabled it, we want to get
        // rid of nosync roles gradually

        foreach ($enrols as $enrol) {
            // Is the user enrolled? We want to sync only really enrolled users
            if (!$DB->record_exists('user_enrolments',
                                    array('userid' => $ra->userid,
                                         'enrolid' => $enrol->id))) {
                continue;
            }
            $context = get_context_instance(CONTEXT_COURSE, $enrol->courseid);
            ...
            $params = array('contextid' => $ra->contextid,
                            'roleid' => $ra->roleid,
                            'userid' => $ra->userid);
            $select = "    contextid = :contextid
                       AND roleid = :roleid
                       AND userid = :userid
                       AND component <> 'enrol_qualification'";
    ...
    public function user_unenrolled($ue) {
        ...
        $sql = "SELECT e.*
                  ...
                  JOIN {enrol} pe
                    ON (pe.courseid = e.customint1
                        AND pe.enrol <> 'qualification'
                        AND pe.courseid = :courseid)
             LEFT JOIN {user_enrolments} pue
                    ON (pue.enrolid = pe.id
                        AND pue.userid = ue.userid)
                 WHERE pue.id IS NULL
                   AND e.enrol = 'qualification'";
        $params = array('courseid' => $ue->courseid,
                        'userid'   => $ue->userid);
...
function enrol_qualification_sync($courseid = null) {
    ...
    if (enrol_is_enabled('qualification')) {
        ...
        $sql = "SELECT DISTINCT pra.roleid, pra.userid, c.id AS contextid, e.id AS enrolid
                  ...
                  JOIN {context} pc
                    ON (pc.id = pra.contextid
                        AND pc.contextlevel = :coursecontext
                        AND pra.component $enabled)
                  JOIN {enrol} e
                    ON (e.customint1 = pc.instanceid
                        AND e.enrol = 'qualification'
                        AND e.status = :statusenabled $onecourse)
                  JOIN {context} c
                    ON (c.contextlevel = pc.contextlevel
                        AND c.instanceid = e.courseid)
             LEFT JOIN {role_assignments} ra
                    ON (ra.contextid = c.id
                        AND ra.userid = pra.userid
                        AND ra.roleid = pra.itemid
                        AND ra.itemid = e.id
                        AND ra.component = 'enrol_qualification')
                 WHERE ra.id IS NULL";
        $params['statusenabled'] = ENROL_INSTANCE_ENABLED;
        ...
        if ($ignored = $qualification->get_config('nosyncroleids')) {
            list($notignored, $xparams) = $DB->get_in_or_equal(explode(',', $ignored),
                                                               SQL_PARAMS_NAMED, 'ig', false);
        ...
        $rs = $DB->get_recordset_sql($sql, $params);
        foreach ($rs as $ra) {
            role_assign($ra->roleid, $ra->userid, $ra->contextid,
                        'enrol_qualification', $ra->enrolid);
        }
        $rs->close();
    }
    ...
    $sql = "SELECT ra.roleid, ra.userid, ra.contextid, ra.itemid
              ...
              JOIN {enrol} e
                ON (e.id = ra.itemid
                    ...
                    AND e.enrol = 'qualification' $onecourse)
              JOIN {context} pc
                ON (pc.instanceid = e.customint1
                    AND pc.contextlevel = :coursecontext)
         LEFT JOIN {role_assignments} pra
                ON (pra.contextid = pc.id
                    AND pra.userid = ra.userid
                    AND pra.roleid = ra.roleid
                    AND pra.component <> 'enrol_qualification' $notignored)
